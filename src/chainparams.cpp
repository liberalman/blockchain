...
public:
    CMainParams() {
        ...
        nDefaultPort = 8333;
        nRPCPort = 8332;
        bnProofOfWorkLimit = CBigNum(~uint256(0) >> 32); // 挖矿难度，前32位是0
        nSubsidyHalvingInterval = 210000; // Subsidy每210,000个区块减半一次，大概每4年发生一次
        ...
        //   vMerkleTree: 4a5e1e
        const char* pszTimestamp = "The Times 05/Mar/2016 socho do it!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN; //初始奖励，比特币的第一个块的奖励是50个btc
        ...
        base58Prefixes[PUBKEY_ADDRESS] = list_of(0); //种子网络连接参数
